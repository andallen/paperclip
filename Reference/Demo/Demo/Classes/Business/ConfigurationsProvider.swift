...
struct ConfigurationsProvider {
  ...
  private static let partTypeConfigurationsPath = Bundle.main.bundlePath.appending(
    "/configurations/")

  static func configurationJson(from part: IINKContentPart) -> String? {
    let configurationPath = Self.configurationPath(from: part)
    guard let configurationJson = try? String(contentsOf: URL(fileURLWithPath: configurationPath))
    else {
      return nil
    }
  ...
  private static func configurationPath(from part: IINKContentPart) -> String {
    ...
    if let configurationFromMetadata = try? part.metadata?.string(
      forKey: Self.configProfileMetadataKey),
      !configurationFromMetadata.isEmpty
    {
      configurationPath =
        Self.partTypeConfigurationsPath + part.type + "/" + configurationFromMetadata + ".json"
    }

...
protocol EditorWorkerLogic {
  ...
  mutating func createNewPart(
    partTypeCreationModel: PartTypeCreationModel, engineProvider: EngineProvider) throws
  func loadNextPart()
...
class EditorWorker: EditorWorkerLogic {
    ...
    throws
  {
    ...
    do {
      if var part: IINKContentPart = try self.currentPackage?.createPart(
        with: partTypeCreationModel.partType.partType)
      {
        // Set its configuration profile
        self.setPartConfigurationProfile(
          part: &part, configuration: partTypeCreationModel.partType.configuration)
        // Then load it
  ...
  func convert(selection: (NSObjectProtocol & IINKIContentSelection)? = nil) throws {
    do {
      if let supportedTargetStates = self.editor?.supportedTargetConversionState(
        forSelection: selection)
      {
        if !supportedTargetStates.isEmpty {
          try self.editor?.convert(
            selection: selection, targetState: supportedTargetStates[0].value)
        }
      }
    } catch {
  ...
  func openFile(file: File, engineProvider: EngineProvider) {
    if let engine: IINKEngine = engineProvider.engine {
      let filePath =
        FileManager.default.pathForFileInIinkDirectory(fileName: file.fileName) as NSString
      self.unloadPart()
  ...
  func enableCaptureStrokePrediction() {
    ...
    try? engine.configuration.set(
      number: Double(frameTimeMs), forKey: "renderer.prediction.duration")
  ...
  private func setPartConfigurationProfile(part: inout IINKContentPart, configuration: String) {
    ...
    try? partMetaData.set(
      string: configuration, forKey: ConfigurationsProvider.configProfileMetadataKey)

...
class ExportViewModel {
  ...
  func export(indexOfMimeTypeSelected: Int) {
      ...
      let mimeTypeValue = self.model?.mimeTypes[indexOfMimeTypeSelected]
    {
      do {
        ...
        try self.editor?.export(
          selection: self.editor?.rootBlock,
          destinationFile: path,
          mimeType: mimeTypeValue.value,
          imagePainter: imagePainter)
        let message = String(
          format: "The content has been exported to %@ successfuly", self.fileName)
        let alertModel = AlertModelHelper.createAlert(
          title: "Export succeeded", message: message, exitAppWhenClosed: false)
  ...
  private func handleExportError() {
    ...
    let alertModel = AlertModelHelper.createAlert(
      title: "Export failed", message: errorText, exitAppWhenClosed: false)
    self.delegate?.exportFinishedWithResult(result: alertModel)
  }

...
class PartTypesViewModel {
  ...
  func loadData() {
    let createBarButtonItem = UIBarButtonItem(
      title: "Create", style: .plain, target: self, action: #selector(create))
    ...
    self.model = PartTypesModel(
      partTypes: partTypeModels,
  ...
  @objc private func create() {
    ...
    {
      self.selectedPartType = PartTypeCreationModel(
        partType: partType, onNewPackage: self.onNewPackage)
    }
  }

...
final class MainCoordinator {
  ...
  func displayEditor(
    editorDelegate: EditorDelegate?, smartGuideDelegate: SmartGuideViewControllerDelegate?
  ) {
  ...
  func createNewPart(cancelEnabled: Bool, onNewPackage: Bool) {
    let viewModel: PartTypesViewModel = PartTypesViewModel(
      engine: EngineProvider.sharedInstance.engine,
      cancelEnabled: cancelEnabled,
      onNewPackage: onNewPackage)
    guard
      let vc = PartTypesTableViewController.instantiate(
        viewModel: viewModel,
        coordinator: self)
    else {
      return
  ...
  func displayExportOptions(editor: IINKEditor?) {
    ...
    guard
      let vc = ExportTableViewController.instantiate(
        viewModel: viewModel,
        coordinator: self)
    else {
      return
  ...
  func displayToolBar(editingEnabled: Bool) {
    guard let mainViewController = self.mainViewController,
      let toolBar = ToolbarViewController.instantiate(
        delegate: mainViewController,
        coordinator: self,
        editingEnabledAtLaunch: editingEnabled)
    else {
      return
    }
  ...
  func displayToolStyle(
    ...
    guard let toolBarVC = self.toolBarViewController,
      let toolStyleVC = ToolStyleViewController.instantiate(
        tool: tool,
        toolStyle: toolStyle,
        delegate: delegate)
    else {
  ...
  func presentAlert(with model: AlertModel) {
    ...
    self.mainViewController?.present(

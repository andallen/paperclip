...
class ToolbarViewController: UIViewController, Storyboarded {
  ...
  static func instantiate(
    delegate: ToolbarProtocol,
    coordinator: MainCoordinator,
    editingEnabledAtLaunch: Bool
  ) -> Self? {
  ...
  private func bindViewModel() {
    ...
    self.viewModel?.$handToolModel.removeDuplicates().sink {
      [weak handButton, handButtonWidthConstraint] model in
      if let model = model {
    ...
    self.viewModel?.$highlighterToolModel.removeDuplicates().sink {
      [weak highlighterButton] model in
      if let model = model {
    ...
    self.viewModel?.$styleButtonEnabled.removeDuplicates().sink {
      [weak changeStyleButton] enabled in
      changeStyleButton?.isEnabled = enabled
    }.store(in: &cancellables)

    // Active Pen Mode
    self.viewModel?.$activePenModel.removeDuplicates().sink {
      [weak activePenLabel, activePenModeSwitch] model in
      if let model = model {
  ...
  @IBAction private func changeStyle(_ sender: Any) {
    guard let viewModel = self.viewModel,
      ...
      let toolStyle = viewModel.getStyleForActiveTool()
    else {
      return
    }
    self.coordinator?.displayToolStyle(
      tool: activeTool,
      toolStyle: toolStyle,

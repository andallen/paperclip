...
class ToolStyleViewController: UIViewController, Storyboarded {
  ...
  static func instantiate(
    tool: IINKPointerTool, toolStyle: ToolStyleModel, delegate: ToolStyleProtocol
  ) -> Self? {
  ...
  private func bindViewModel() {
    ...
    self.viewModel?.$widthSegmentedControlModel.removeDuplicates().sink {
      [weak widthSegmentedControl] model in
      if let model = model {

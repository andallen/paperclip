...
class MainViewModel: NSObject {
  ...
  init(
    delegate: MainViewControllerDisplayLogic?,
  ...
  func checkEngineProviderValidity() {
    if self.engineProvider.engine == nil {
      self.errorAlertModel = AlertModelHelper.createAlert(
  ...
  func createNewPart(partTypeCreationModel: PartTypeCreationModel, engineProvider: EngineProvider) {
    do {
      try self.editorWorker.createNewPart(
        partTypeCreationModel: partTypeCreationModel, engineProvider: engineProvider)
    } catch {
  ...
  func moreActions(barButtonIdem: UIBarButtonItem) {
    ...
    self.moreActionsAlertModel = AlertModel(
      title: "More actions", alertStyle: .actionSheet, actionModels: actions)
  }
  ...
  private func createMoreMenu(
    with content: NSObjectProtocol & IINKIContentSelection,
    position: CGPoint,
    sourceView: UIView?,
    sourceRect: CGRect
  ) {
        ...
        if type == "Text" {
          ...
          let action = ActionModel(actionText: actionTitle) { [weak self] action in
            let inputAction = ActionModel(actionText: actionTitle) { [weak self] action in
              do {
                try self?.editorWorker.addTextBlock(
                  position: position, data: self?.addTextBlockValue ?? "")
              } catch {
                self?.handleDefaultError(errorMessage: error.localizedDescription)
              }
            }
            self?.inputAlertModel = AlertModel(
              title: actionTitle,
    ...
    if actionModels.count > 0 {
      self.menuAlertModel = AlertModel(
  ...
  private func createFormatTextMenu(
    selection: NSObjectProtocol & IINKIContentSelection,
    sourceView: UIView?,
    sourceRect: CGRect
  ) {
    DispatchQueue.main.async {
      ...
      for format in formats {
        let action = ActionModel(actionText: TextFormatHelper.name(for: format.value)) {
          [weak self] action in
          do {
  ...
  private func handleDefaultError(errorMessage: String) {
    self.errorAlertModel = AlertModelHelper.createDefaultErrorAlert(
    ...
    for blockId in blockIds {
        ...
        && block?.parent?.type != "Text"
      {
        do {
          ...
          let solveAsStrokes = try mathSolver.diagnostic(
            forBlock: blockId, task: "numerical-computation", overrideConfiguration: configStrokes)
          let solveAsGlyphs = try mathSolver.diagnostic(
            forBlock: blockId, task: "numerical-computation", overrideConfiguration: configGlyphs)

          if solveAsStrokes.value.rawValue == IINKMathDiagnostic.allowed.rawValue
            && solveAsGlyphs.value.rawValue == IINKMathDiagnostic.allowed.rawValue
          {  // not already solved as strokes or glyphs
            let conversionState = try editor.conversionState(forSelection: block)
            let config =
              ((conversionState.value.rawValue & IINKConversionState.handwriting.rawValue) != 0)
              ? configStrokes : configGlyphs

            try mathSolver.apply(
              forBlock: blockId, action: "numerical-computation", overrideConfiguration: config)
          }
        } catch {
          self.handleEditorError(error: error)
        }
...
extension MainViewModel: SmartGuideViewControllerDelegate {
  ...
  func smartGuideViewController(
    _ smartGuideViewController: SmartGuideViewController!,

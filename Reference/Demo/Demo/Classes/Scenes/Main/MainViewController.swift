...
class MainViewController: UIViewController, Storyboarded {
  ...
  var partTypeToCreate: PartTypeCreationModel? {
    didSet {
      if let partTypeToCreate = self.partTypeToCreate {
        self.viewModel?.createNewPart(
          partTypeCreationModel: partTypeToCreate, engineProvider: EngineProvider.sharedInstance)
      }
    }
  }
  var fileToOpen: File? {
  ...
  override func viewDidLoad() {
    ...
    self.coordinator?.displayEditor(
      editorDelegate: self.viewModel, smartGuideDelegate: self.viewModel)
  }
  ...
  private func bindViewModel() {
    ...
    self.viewModel?.$addPartItemEnabled.assign(to: \.isEnabled, on: self.addPartBarButtonItem)
      .store(in: &cancellables)
    self.viewModel?.$editingEnabled.assign(to: \.isEnabled, on: self.convertBarButtonItem).store(
      in: &cancellables)
    self.viewModel?.$editingEnabled.assign(to: \.isEnabled, on: self.zoomInBarButtonItem).store(
      in: &cancellables)
    self.viewModel?.$editingEnabled.assign(to: \.isEnabled, on: self.zoomOutBarButtonItem).store(
      in: &cancellables)
    self.viewModel?.$editingEnabled.assign(to: \.isEnabled, on: self.moreBarButtonItem).store(
      in: &cancellables)
    self.viewModel?.$editingEnabled.sink { [weak self] enabled in
      self?.coordinator?.enableEditing(enable: enabled)
    }.store(in: &cancellables)
    self.viewModel?.$previousButtonEnabled.assign(
      to: \.isEnabled, on: self.previousPartBarButtonItem
    ).store(in: &cancellables)
    self.viewModel?.$nextButtonEnabled.assign(to: \.isEnabled, on: self.nextPartBarButtonItem)
      .store(in: &cancellables)
  ...
  func injectEditor(editor: EditorViewController) {
    ...
    self.longPressgestureRecognizer = UILongPressGestureRecognizer(
      target: self, action: #selector(longPressGestureRecognizerAction))
  ...
  @objc private func longPressGestureRecognizerAction() {
    ...
    if let sourceView = longPressgestureRecognizer.view {
      self.viewModel?.handleLongPressGesture(
        state: longPressgestureRecognizer.state, position: position, sourceView: sourceView)
    }
  }
}

...
actor SkillCloudClient: SkillCloudClientProtocol {
  ...
  private func parseSections(_ value: Any?) -> [LessonSection] {
    guard let array = value as? [[String: Any]] else { return [] }
    return array.compactMap { dict in
      guard let heading = dict["heading"] as? String,
            let content = dict["content"] as? String else { return nil }
      return LessonSection(heading: heading, content: content)
    }
  }

  // Parses exercises array from JSON.
  private func parseExercises(_ value: Any?) -> [LessonExercise]? {
    guard let array = value as? [[String: Any]] else { return nil }
    let exercises = array.compactMap { dict -> LessonExercise? in

...
final class NotebookEditorViewController: UIViewController {
  ...
  override func viewDidLayoutSubviews() {
    ...
    do {
      ...
      if let renderer = displayViewModel.renderer {
        ...
        appLog(
          "🧭 NotebookEditorViewController.viewDidLayoutSubviews renderer viewScale \(beforeScale)→\(renderer.viewScale) viewOffset \(beforeOffset)→\(renderer.viewOffset)"
        )
      }
    } catch {
  ...
  private func setupMyScript() {
    Task {
      do {
        ...
        guard let editor = engine.createEditor(renderer: renderer, toolController: toolController)
        else {
        ...
        do {
          try editor.toolController.set(
            tool: IINKPointerTool.toolPen, forType: IINKPointerType.touch)
        ...
        do {
          try editor.toolController.set(
            style: "color:#000000;-myscript-pen-width:1.5", forTool: IINKPointerTool.toolPen)
        } catch {
        ...
        await MainActor.run {
          ...
          if sizePx.width > 0 && sizePx.height > 0 {
            do {
              ...
              renderer.viewOffset = .zero
              appLog(
                "🧭 NotebookEditorViewController.setupMyScript renderer viewScale \(beforeScale)→\(renderer.viewScale) viewOffset \(beforeOffset)→\(renderer.viewOffset)"
              )
            } catch {
              appLog("❌ NotebookEditorViewController: Failed to set view size: \(error)")
            }
  ...
  private func loadDocument() {
    ...
    Task {
      do {
        ...
        appLog(
          "🧭 NotebookEditorViewController.loadDocument partCount=\(partCount) partReady=\(part != nil) partType=\(partType) partId=\(partId)"
        )
  ...
  private func waitForEditorIdle(context: String) async {
    ...
    let watchdog = Task.detached { [start] in
      ...
      appLog(
        "⚠️ NotebookEditorViewController.waitForIdle still running context=\(context) elapsed=\(String(format: "%.2f", elapsed))"
      )
    }

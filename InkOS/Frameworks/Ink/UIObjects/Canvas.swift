...
extension Canvas: IINKICanvas {
  ...
  func setFillColor(_ color: UInt32) {
    ...
    self.fontAttributeDict[NSAttributedString.Key.foregroundColor] = IInkUIRefImplUtils.uiColor(
      rgba: color)
  }
  ...
  func setDropShadow(_ xOffset: Float, yOffset: Float, radius: Float, color: UInt32) {
    if color != 0 {
      ...
      self.context?.setShadow(
        offset: size, blur: CGFloat(radius), color: IInkUIRefImplUtils.uiColor(rgba: color).cgColor)
    }
  }
  ...
  func startGroup(_ identifier: String, region: CGRect, clip clipContent: Bool) {
    if clipContent {
      ...
      self.context?.clip(
        to: CGRect(
          x: region.origin.x, y: region.origin.y, width: region.width, height: region.height))
    }
  }
  ...
  func drawText(_ label: String, anchor origin: CGPoint, region rect: CGRect) {
    guard let context = self.context,
      let font = UIFont.fontFromStyle(style: self.style, string: label)
    else {
      return
    }
    self.fontAttributeDict[NSAttributedString.Key.font] = font
    let attrString: NSAttributedString = NSAttributedString(
      string: label, attributes: self.fontAttributeDict)
  ...
  func blendOffscreen(_ offscreenId: UInt32, src: CGRect, dest: CGRect, color: UInt32) {
    guard let buffer: CGLayer = self.offscreenRenderSurfaces?.getSurfaceBuffer(forId: offscreenId),
      let scale = self.offscreenRenderSurfaces?.scale
    else { return }
